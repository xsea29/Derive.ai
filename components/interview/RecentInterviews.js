...
export function RecentInterviews() {
  return (
    <motion.div
      ...
      className="rounded-lg sm:rounded-xl bg-card border border-border card-elevated overflow-hidden"
    >
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-4 sm:p-5 border-b border-border">
        ...
        <Link href="/interview" className="w-full sm:w-auto">
          <Button variant="ghost" size="sm" className="gap-1.5 text-muted-foreground text-xs sm:text-sm h-9 px-3 sm:px-3 tap-target w-full sm:w-auto justify-center sm:justify-start">
            View all
        ...
        {interviews.map((interview, index) => {
          ...
          return (
            <motion.div
              ...
              className="flex items-start sm:items-center gap-3 sm:gap-4 p-3 sm:p-4 hover:bg-muted/30 transition-colors cursor-pointer min-h-[80px] sm:min-h-auto"
            >
              <div className="flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-lg bg-primary/10 shrink-0 tap-target">
                <TypeIcon className="h-4 w-4 sm:h-5 sm:w-5 text-primary" />
              </div>

              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <h3 className="font-medium text-sm sm:text-base truncate">{interview.jobTitle}</h3>
                  <StatusBadge status={interview.status} />
                </div>
                <div className="flex items-center gap-1.5 sm:gap-3 mt-1 text-xs sm:text-sm text-muted-foreground flex-wrap">
                  <span className="hidden sm:inline">{interview.department}</span>
                  <span className="hidden sm:inline">•</span>
                  <span className="truncate">{typeLabels[interview.type].replace('AI ', '').replace(' Interview', '')}</span>
                  <span>•</span>
                  <span className="truncate">{interview.createdAt}</span>
                </div>
              ...
              <Button variant="ghost" size="icon" className="text-muted-foreground shrink-0 h-9 w-9 sm:h-9 sm:w-9 tap-target">
