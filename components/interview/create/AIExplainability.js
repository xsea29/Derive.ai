...
export default function AIExplainability({ data = {} }) {
  ...
  const getSkillsToEvaluate = () => {
    ...
    if (data.title && (data.title.toLowerCase().includes('frontend') || data.title.toLowerCase().includes('react'))) {
      return ['React', 'JavaScript', 'CSS', 'Problem Solving', 'Communication'];
    }
    if (data.title && data.title.toLowerCase().includes('backend')) {
      ...
      <Card className="bg-accent/30 border-primary/20">
        <CardContent className="p-4 md:p-5">
          <div className="flex items-start gap-3 mb-4">
            <div className="h-8 w-8 rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 flex items-center justify-center shrink-0">
          ...
          <div className="mt-4 pt-3 border-t border-border/50">
            <p className="text-xs text-muted-foreground">
              <span className="font-medium">AI Note:</span> These questions are generated to evaluate {data.skills && data.skills.length > 0 ? data.skills.slice(0, 2).join(', ') : 'core competencies'}, problem-solving, and communication based on the job description and experience level.
            </p>
          </div>
        </CardContent>
      </Card>
