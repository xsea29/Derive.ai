...
export default function CandidateImport({ candidates = [], setCandidates, onNext }) {
  ...
  const handleApplyTemplate = (mapping) => {
  ...
  const handlePasteEmailsChange = (text) => {
  ...
  const tabs = [
    { id: "csv", label: "Upload CSV / Excel", icon: FileSpreadsheet, highlighted: true },
    { id: "paste", label: "Paste Emails", icon: Mail },
    { id: "ats", label: "ATS Integration", icon: Link2, comingSoon: true },
  ];
